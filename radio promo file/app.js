...
async function loadBeds() {
  ...
  try {
    ...
    if (!beds.length) {
      box.innerHTML = `<div class="notice warn">${esc(note || 'No music beds uploaded yet.')} Spots will ship as a dry voice read, which sounds thin next to produced commercials. Upload licensed beds to your Cloudinary bed folder.</div>`;
      return;
    }
    box.innerHTML = `
      ...
      <div class="options">
        ...
        ${beds.map((b) => radioCard('bed', b.publicId, b.name, b.seconds ? `${Math.round(b.seconds)}s loop` : '', chosen === b.publicId)).join('')}
      </div>`;
    box.querySelectorAll('input[name=bed]').forEach((input) => input.addEventListener('change', async () => {
      const bed = state.beds.find((b) => b.publicId === input.value) || null;
  ...
  if (p.approvalRequest) {
    ...
    stage.innerHTML = panel('Station 108.1', 'Sent for approval', `
      ...
      <div class="actions">
        ...
        <a class="btn ghost" href="/library.html">Open the library</a>
        <span class="spacer"></span>
        <a class="btn" href="/">Start another project</a>
      </div>`);
